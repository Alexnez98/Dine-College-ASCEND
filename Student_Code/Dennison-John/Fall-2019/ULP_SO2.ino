 ...
 *  This is for the Dine College ASCEND ULP code for the SO2 sensor-DJ
 *  Functions include: 
...
// this is the modified version to fit SO2 SPEC sensors for ASCEND,
const int C1 = A0;
const int T1 = A3;
//const int C2 = A1;
//const int T2 = A4;

const float Sf1 = -45.41; //nA/ppm, change sensitivity to SO2
...
//NO2 sensor1(C1, T1, Sf1); // SO2 or CO
//CO sensor2(C2, T2, Sf2);  // turn this on for two sensors
//H2S sensor3(C3, T3, Sf3); //Example H2S, turn this on for three sensors, gotta declare he constant if you want more than one senor to an UNO
...
void setup() {
  ...
  Serial.flush();
  Serial.begin(9600);    // initialize serial communications at 9600 bps: ALWAYS CHECK IF THE SD CARD IS FORMATTED 
  ...
  Serial.println("Not Removing SO2_Test.TXT...");
  //SD.remove("SO2_Test.TXT"); // remove is a function in SD library to delete a file
  if (SD.exists("SO2_Test.TXT")) { //If the file still exist display message exist
    Serial.println("The SO2_Test.TXT exists.");
  } else { //If the file was successfully deleted display message that the file was already deleted.
    Serial.println("The SO2_Test.TXT doesn't exist.");
  }

  myFile = SD.open("SO2_Test.TXT", FILE_WRITE); // Create a new file after removing the old file.
  
 
  // if the file opened okay, write to it:
  if (myFile) {
    Serial.print("Writing to SO2_Test.TXT...");
