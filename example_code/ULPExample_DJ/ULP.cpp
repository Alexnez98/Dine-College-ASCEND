...
int  ULP::getTemp(int n, char* U) {
  ...
  float TempC = (_Ts / _Vsup) * Volts - _Tb;
...
float  ULP::setVref (long R1, long R2, long R3, int bias) {
	if (bias>0){
...
float  ULP::getConc(int n, float t) {
...
H2S::H2S (int a, int b, float c): ULP(a, b, c) { //works
